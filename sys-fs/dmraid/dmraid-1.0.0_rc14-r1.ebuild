...
inherit linux-info flag-o-matic multilib
...
src_compile() {
	...
	cd "${S}"/lib
	emake libdmraid.so || die "emake libdmraid.so failed"
}
...
src_install() {
	...
	if ! use static; then
		cd "${S}"/lib
		dolib libdmraid.so
		dosym /usr/$(get_libdir)/libdmraid.so /usr/$(get_libdir)/libdmraid.so.${PV/_/.}
	fi
}
